describe "doctors", type:  :feature do
  before do
    @merideth = Doctor.create!({name: "Meredith Grey", department: "Internal Medicine"})
    @bart = Patient.create!(name: "Bart Simpson", age:10 )
    Appointment.create(appointment_datetime: DateTime.new(2016, 01, 11, 20, 20, 0), patient_id: @bart.id, doctor_id: @merideth.id)
  end

  describe "#show page" do
    it "shows all of a doctor's appointment times in a human readable format" do
      visit doctor_path(@merideth)
